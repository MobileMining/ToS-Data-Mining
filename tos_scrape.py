...
def card_to_list(i): # -> list
	...
	card_active_skill_title = str(txt(i[3][1][0]).encode('ascii','ignore')) #this is needed as there is a seperate br tag for <a href>
	card_active_skill_desc = str(txt(i[3][1])[len(card_active_skill_title):-1].encode('ascii','ignore')) #seperate title from description
	...
	return [card_id,card_name,card_attribute,card_element,
			...
			card_active_skill_title,card_active_skill_desc,
...
#----- INDEXING -----
index_db = [] #this will be for the used to scroll through the site
xpath_index = '//*[@id="mw-content-text"]/table[1]'
index_tree = tree.xpath(xpath_index)

for row in range(1,2): #cannot start from 0 as it is just a string
	for col in range(1):
...
card_db.append(['ID','Name','Attribute','Race','Max Lvl','Exp Curve','Max Exp','Rarity','Cost','Series',
				...
				'Active Skill','Active Skill Description','Leader Skill','Leader Skill Description'])
